...
import edu.stevens.cs522.base.TimestampUtils;
...
public class PeerContract extends BaseContract {
    ...
    public static Instant getTimestamp(Cursor cursor) {
        if (timestampColumn < 0) {
            timestampColumn = cursor.getColumnIndexOrThrow(TIMESTAMP);
        }
        return TimestampUtils.getTimestamp(cursor, timestampColumn);
    }

    public static void putTimestamp(ContentValues out, Instant timestamp) {
        TimestampUtils.putTimestamp(out, TIMESTAMP, timestamp);
    }
